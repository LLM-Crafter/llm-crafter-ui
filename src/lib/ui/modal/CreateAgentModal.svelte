<script lang="ts">
	...
	let promptMode: 'system_prompt' | 'prompt_sections' = 'system_prompt';
	let promptSections = {
		identity_and_tone: '',
		tools_and_apis: '',
		conversation_flow: '',
	...
	>
		...
		<div class="flex-1 overflow-y-auto p-6">
					...
					{:else}
						...
						<div class="space-y-4 rounded-lg border border-violet-500/20 bg-violet-500/5 p-4">
							<p class="text-xs text-gray-400">
								...
								Define each section independently. The V2 engine composes them into the final prompt
								automatically. All sections are optional.
							</p>
							<div>
								<label
									class="mb-1.5 flex items-center space-x-2 text-sm font-medium text-gray-300"
									for="ps-create-identity"
								>
									<i class="fas fa-user-circle text-blue-400"></i><span>Identity &amp; Tone</span>
								</label>
								<textarea
									id="ps-create-identity"
			...
			{:else if step === 4}
				...
				<div class="space-y-4">
					<div>
						<h4 class="text-lg font-semibold text-gray-100">GDPR & Data Privacy</h4>
						<p class="text-sm text-gray-400">
							Configure data privacy settings to comply with GDPR regulations
						</p>
					</div>
					...
					>
						<div>
							<h5 class="font-medium text-gray-100">Encrypt Messages</h5>
							<p class="text-sm text-gray-400">
					...
					<div class="rounded-lg border border-gray-700 bg-gray-800 p-4">
						<label class="mb-2 block text-sm font-medium text-gray-300" for="gdpr-retention-days">
							Data Retention (days)
						</label>
						<input
							id="gdpr-retention-days"
							type="number"
							min="1"
